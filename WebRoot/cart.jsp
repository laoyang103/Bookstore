...
<c:if test="${empty notices && empty isNull }">
     <jsp:forward page="getNotices.do?url=cart.jsp"></jsp:forward>
</c:if>

<c:if test="${empty categories}">
	<jsp:forward page="getParentCategories.do?url=cart.jsp"></jsp:forward>
</c:if>
<c:if test="${empty products}">
   <jsp:forward page="getProductsBySum.do?url=cart.jsp"></jsp:forward>
</c:if>
<html>
<head>
<title>奋斗电子商城</title>
...
<table width="100%" height="126"  border="0" cellpadding="0" cellspacing="0">
       ...
       <table id="cartTable" width="92%" height="48" border="0" align="center" cellpadding="0" cellspacing="0">
         
      </table>
      <table id="bottomTable" width="582" height="52" border="0" align="center" cellpadding="0" cellspacing="0">   
         <tr align="center" valign="middle">
           <td height="21" colspan="2"> 
           		<a href="index.jsp">继续购物</a> |
           	<c:if test="${empty user }">
            	<a href="javascript:showMsg('<font color=yellow>您还没登录,购物请先登录!</font>');">去收银台结账</a> |
            </c:if>
            <c:if test="${not empty user }">
            	<a href="cart_checkout.jsp">去收银台结账</a> |
            </c:if>
             	<a class="button" onclick="clearCart()">清空购物车</a>
           </td>
         </tr>
      </table>
						</td>
