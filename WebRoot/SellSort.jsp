...
<c:if test="${empty notices && empty isNull }">
     <jsp:forward page="getNotices.do?url=SellSort.jsp"></jsp:forward>
</c:if>
<c:if test="${empty products}">
   <jsp:forward page="getProductsBySum.do?url=SellSort.jsp"></jsp:forward>
</c:if>
            ...
            <tr valign="top">
              ...
              <c:set var="colum" value="1"></c:set>
			      ...
			      <table width="96%" height="30"  border="0" cellpadding="0" cellspacing="0" class="tableBorder_B_dashed">
                    <tr>
                      <td width="5%" align="center">${colum }.</td>
                      <td width="45%"><a href="showProductDetail.do?id=${product.id }">${product.name}</a></td>
                      <td height="20">￥${product.normalprice} </td>
                      <td width="30%" align="center">
                      <c:choose>
                          <c:when test="${product.ncount > 0}">
                           <input type="button" class="button" onClick="addCart(${product.id})" value="购买"></c:when>                          
