...
<c:if test="${empty notices && empty isNull }">
     <jsp:forward page="getNotices.do?url=sale.jsp"></jsp:forward>
</c:if>

<c:if test="${empty categories}">
	<jsp:forward page="getParentCategories.do?url=sale.jsp"></jsp:forward>
</c:if>
<c:if test="${empty products}">
   <jsp:forward page="getProductsBySum.do?url=sale.jsp"></jsp:forward>
</c:if>


<c:if test="${empty specialproducts}">
   <jsp:forward page="showSpecialProducts.do"></jsp:forward>
</c:if>
            ...
            <tr valign="top">
			  ...
			  <c:forEach items="${specialproducts}" var="specialproduct">
                  <table width="98%" height="79"  border="0" cellpadding="0" cellspacing="0" class="noborder">
                    <tr>
                      <td width="37%" height="95" rowspan="4" align="center"><img src="images/goods/${specialproduct.img}.jpg" width="150" height="100"></td>
                      <td width="63%" height="27" class="font2"><a href="showProductDetail.do?id=${specialproduct.id}">${specialproduct.name}</a></td>
                    </tr>
                    <tr>
                      <td height="27" style="text-decoration:line-through;">市场价：￥${specialproduct.normalprice}</td>
                    </tr>
                    <tr>
                      <td height="27" style="color:#FF0000">商城价：￥${specialproduct.memberprice} </td>
                    </tr>                   
                    <tr>
                      <td><input type="button" class="button" value="购买" onclick="addCart(${specialproduct.id})">&nbsp;&nbsp;
						 <input type="button" class="button" value="收藏" onclick="addFavorite(${specialproduct.id})">
						</td>
                    </tr>
                  </table>
