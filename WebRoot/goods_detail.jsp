...
<c:if test="${empty notices && empty isNull }">
     <jsp:forward page="getNotices.do?url=goods_detail.jsp"></jsp:forward>
</c:if>

<c:if test="${empty categories}">
	<jsp:forward page="getParentCategories.do?url=goods_detail.jsp"></jsp:forward>
</c:if>
<c:if test="${empty products}">
   <jsp:forward page="getProductsBySum.do?url=goods_detail.jsp"></jsp:forward>
</c:if>
...
value="搜索"></td>
      ...
      <table width="100%"  border="0" cellspacing="0" cellpadding="0">
        <tr>
          <td height="189" valign="top"><table width="591"  border="0" cellspacing="0" cellpadding="0" height="529">
            <tr>
              <td height="40" background="images/center01.gif">&nbsp;</td>
              </tr>
            <tr valign="top">
              <td height="134" >
                  <table width="96%" height="79"  border="0" cellpadding="0" cellspacing="0" class="noborder">
                    ...
                    <tr>
                      <td width="37%" height="95" rowspan="4" align="center"><img src="images/goods/${product.img }.jpg" width="150" height="100"></td>
                      <td width="63%" style="text-decoration:line-through;">市场价：￥${product.normalprice}</td>
                    </tr>
                    <tr>
                      <td style="color:#FF0000">商城价：￥${product.memberprice} </td>
                    </tr>
                    <tr>
                      <td>
						 <input type="button" class="button" value="购买" onclick="addCart(${product.id} )">&nbsp;&nbsp;
						 <input type="button" class="button" value="收藏" onclick="addFavorite(${product.id})">
					  </td>
                    </tr>
                    <tr>
                      <td>
						<c:choose>
                          <c:when test="${product.ncount > 0}">
                                                                现在有货</c:when>                          
                          <c:otherwise>缺货<br> </c:otherwise>
                         </c:choose>
					  </td>
                    </tr>
                    </table>
                    ...
                    <tr>
                      ...
                      <td>
                      	<c:if test="${not empty user }">
                        	<input type="button" class="button" value="我要评论" onclick="$('#commentInputBox').slideDown();"><br>
                        </c:if>
                        <c:if test="${empty user }">
                        	<input type="button" class="button" value="我要评论" onclick="showMsg('没登陆是不能评论的哦~~');"><br>
                        </c:if>  
					  </td>
                    </tr>
                  </table>
<c:if test="${not empty comments }">                    
                  <table width="587" height="20" border="1" cellpadding="0" cellspacing="0" bordercolor="#FFFFFF" bordercolordark="#CCCCCC" bordercolorlight="#FFFFFF">
              <tr bgcolor="#eeeeee">
                <td width="10%" align="center">编号</td>
                <td width="15%" height="20" align="center">用户名称</td>
                <td width="60%" align="center">评论内容</td>
                <td width="15%" align="center">评论日期</td>
              </tr>
              <!-- 迭代商品 -->           
             <c:set var="colum" value="1"></c:set>
			<c:forEach items="${comments}"  var="comment">
              <tr id="" style="padding:5px;">
                <td align="center">${colum }.</td>
                <td height="25" align="center">${comment.userName}</td>                
                <td align="center">${comment.content}</td>
                <td align="center">${comment.cdate }</td>
...
此商品暂无评论
            ...
            <c:if test="${not empty user }">
            <table style="display:none" id="commentInputBox" width="100%"  border="0" align="center" cellpadding="-2" cellspacing="-2" bordercolordark="#FFFFFF">
                  
				  <tr>
                    <td width="16%" >评论内容</td>
                    <td width="84%" ><textarea title="请输入您对商品的评论" id="descr" name="descr" cols="50" rows="5"></textarea></td>
                  </tr>
                  <tr align="center">
                    <td height="30" colspan="3">
                        <input type="button" class="button" value="提交" onClick="submitComment($('#descr').val(),${product.id})">
                        &nbsp;
                        &nbsp;
                        <input type="button" class="button" value="取消" onClick="$('#commentInputBox').hide();">                    </td>
                    </tr>
                </table>
                </c:if>
            <tr>
